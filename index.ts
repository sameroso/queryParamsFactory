...
> {
  ...
  static create<K extends Record<string, string> = Record<string, string>>(
    queryUrl = ""
  ): urlSearchParamsBuilder<K> {
    return new urlSearchParamsBuilder<K>(queryUrl);
  }
   ...
   *
   */
  removeParam(key: keyof T): urlSearchParamsBuilder<T> {
   ...
   *  console.log(urlSearchParamsWithRemovedParamList) => "key1=value1&key4=value4"
   */
  removeParamList(paramsList: (keyof T)[]): urlSearchParamsBuilder<T> {
    paramsList.forEach((key) => {
      this.urlSearchParams.delete(key as string);
    });
    return this;
...
const { addParam, compose, getParamList } =
  urlSearchParamsFactory<{ samer: string }>("?samin=dasGata");

const newUrl = compose(
  () => addParam({ key: "samer", value: "sameroso" }),
  () => addParam({ key: "para", value: "tudo" })
);

const ty = getParamList(["samer"]);

console.log(newUrl);

const x = urlSearchParamsBuilder
  .create("?samin=dasGata")
  .addParamList([{ key: "doido", value: "foi" }])
  .getParam("doido");
